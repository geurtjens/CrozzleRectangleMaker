...
import Foundation
public class PacmanCalculatorV2 {
    ...
    public static func ExecuteAllSerial(scoreMin: Int, includeBreakdown: Bool = true) -> Int  {
        ...
        for game in gameList.games {
            ...
            if includeBreakdown {
                print("PacmanCalculatorV2.Execute: \(game.gameId), count: \(result.count)")
            }
            count += result.count
        }
        ...
        print("PacmanCalculatorV2.Execute: \(count) records found in \(duration)")
        return count
    }
